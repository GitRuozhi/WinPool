...
- 在 V0.3 期间，不实现或启用真实磁盘、分区、卷、存储池、存储层或虚拟磁盘的创建、删除、初始化、格式化、调整、修复或等价修改。
...
- V0.36 是当前本地实施版本；自动质量门和 staging 结果记录在 `CHANGELOG.md`，继承的原生/人工用例继续保持 `unverified`，不得将其写为通过。
- 已实施的 V0.36 Plan 归档在 `docs/Archive/V0.36`；当前不存在活动 `docs/Plan.md`。用户本次仅授权本地 Git checkpoint；推送、tag、GitHub Release、二进制上传和部署仍未授权。
...
- 未运行或不可用的门不得报告为通过。
- V0.3 不允许用真实硬件结构修改作为验证方法。V0.5 或以后受控验证必须遵守真实修改授权规则。
