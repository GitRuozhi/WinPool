...
- 在 V0.4 期间，不实现或启用真实磁盘、分区、卷、存储池、存储层或虚拟磁盘的创建、删除、初始化、格式化、调整、修复或等价修改。
- V0.5 是最早可增加真实存储结构修改路径的阶段。该路径必须使用经过审阅的类型化操作、精确目标校验、预览、审计记录以及下列授权规则；自由形式存储命令仍然禁止。
- 开发 Agent 每次执行实际存储修改前，必须向开发者请求针对该操作及其目标的明确批准。先前的、宽泛的或隐含的批准均不足够。
- 产品中，用户在当前会话显式选择“本机真实修改”选项，即授权执行 V0.5 的受控真实操作。提权或仅选择 Real 模式不构成授权；该选项不得预选，也不得把同意持久化。
- 模拟仍是存储结构变化的默认实现路径；即使界面处于 Real 模式，用户完成真实修改授权流程前也必须保持模拟。
...
- 产品版本在新产品线采用 `Va.b`，同一产品线的非零一位迭代可采用 `Va.bc`：`a` 为大版本，`b` 为小版本。
- `Va.b` / `Va.bc` 是唯一项目版本体系。.NET/Windows 必需的数字字段只能是机械派生的编译元数据，不得命名或记录为另一套项目版本。
- 架构与路线图通常只写到 `Va.b`。
- `c=8` 或 `c=9` 时提醒开发者控制范围；不得创建 `c=10`。
- 普通非零 `c` 迭代需要本地提交；除非明确授权，不推送、不打 tag、不发布。
- V0.41 是当前本地实施版本；自动质量门和构建结果记录在 `CHANGELOG.md`，继承的原生/人工用例继续保持 `unverified`，不得将其写为通过。
- 已实施的 V0.39 Plan 归档在 `docs/Archive/V0.39` 和
  `docs/Archive/V0.39-final-correction`。已确认的 V0.41 计划归档在
  `docs/Archive/V0.41`。实施已经
  完成；用户另行授权前，不得执行 commit、push、tag、Release、二进制上传或部署。
  V0.39 保留为历史 tag 和 GitHub Release 记录；commit、push、tag、Release、二进制上传
  或部署均需单独授权。
- 用户允许 V0.41 完整重置并重新生成本开发机现有的 WinPool 偏好和数据库；V0.4 到 V0.41
  不要求数据兼容。用户要求开始实施前不得执行重置；取得授权后须先停止全部 WinPool 进程、核对
  准确数据根，并把旧状态移到项目根 `Rubbish`，不得直接删除。
...
- 未运行或不可用的门不得报告为通过。
- V0.41 不允许用真实硬件结构修改作为验证方法。V0.5 或以后受控验证必须遵守真实修改授权规则。
