...
- V0.4 是当前本地实施版本；自动质量门和构建结果记录在 `CHANGELOG.md`，继承的原生/人工用例继续保持 `unverified`，不得将其写为通过。
- 已实施的 V0.39 Plan 归档在 `docs/Archive/V0.39` 和
  `docs/Archive/V0.39-final-correction`；当前不存在活动 `docs/Plan.md`。V0.3 最终
  修正已经完成，正常开发进入 V0.4。V0.39 保留为历史 tag 和 GitHub Release 记录；新的
  tag、Release、二进制上传或部署均需单独授权。
...
- 未运行或不可用的门不得报告为通过。
- V0.4 不允许用真实硬件结构修改作为验证方法。V0.5 或以后受控验证必须遵守真实修改授权规则。
