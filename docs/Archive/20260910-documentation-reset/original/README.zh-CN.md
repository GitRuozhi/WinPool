...
当前产品版本为 **V0.47**。版本源是 `Directory.Build.props`。最终结果见
[变更记录](docs/CHANGELOG.zh-CN.md)。

## 能力

WinPool 提供存储拓扑、聚焦的操作工作区、存储结构编辑与磁盘分区编辑（模拟），以及监控。
当前交付为无打包、自包含的 Windows x64 便携式应用。对外最低受支持操作系统为
Windows 10 22H2 x64。
...
- [当前计划](docs/Plan.zh-CN.md)：活动正式阶段（若存在）。
