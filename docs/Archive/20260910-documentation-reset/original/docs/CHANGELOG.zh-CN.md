...
## V0.47 存储结构编辑与磁盘分区编辑 — 2026-09-07

### Changed
- 产品版本为 **V0.47**。
- 编辑页改为两页模拟编辑：存储结构编辑（原下半区）和磁盘分区编辑（原上半区）。
- 存储结构编辑页仍是一池一块虚拟磁盘、可选一个用户分区。创建磁盘时创建分区
  默认打开。允许删除虚拟磁盘，含最后一块。
- 开发者结束本拆页阶段。计划冻结于
  [Archive/V0.47-editor-pages](Archive/V0.47-editor-pages/Plan.md)。存储结构
  控件排布是当前活动 Plan，不是本条结果。

### Verification
- 2026-09-07：拆页实现已在 `main`（`ca03c69`、`3305261`）；七页壳和编辑页的
  架构覆盖在 `7316075`。本次文档收口未重跑这些测试。
- 原生 PE8（启动 → 各新页 → 进程仍活 → 无新崩溃日志）：`unverified`。
- 真实存储结构变更：`not_required`，仍被拒绝。

### Known Limitations
- 真实存储结构变更仍被拒绝。
- 存储结构页右上、右下仍是原编辑页控件；控件 Plan 尚未执行。
- 两个新页的原生打开仍为 `unverified`。

## V0.46 编辑下区多虚拟磁盘横排 — 2026-09-07

### Changed
- 编辑页下区在一个池有多块虚拟磁盘时，改用与管理页相同的隐藏
  `VirtualDiskGroup` Flow，卡片横排，不再在池里纵排。

### Verification
- 2026-09-07：`EditWorkspaceTests` 通过 28，失败 0，跳过 0。

### Known Limitations
- 真实存储结构变更仍被拒绝。

## V0.46 同池未划层回层提交 — 2026-09-07

### Changed
- 执行修改现在会提交：把仍留在同一池里的未划层磁盘拖回对应介质层。确认池
  属性在写入名称和参数之后，会保留这些尚未提交的成员变更，而不再把工作
  副本整份打回上次已提交快照。

### Verification
- 2026-09-07：同池回层与成员还原的 EditWorkspace、模拟操作测试通过 37，
  失败 0，跳过 0。编辑页提交路径的架构检查通过。

### Known Limitations
- 真实存储结构变更仍被拒绝。
- 确认池属性仍然不创建成员关系。未划层磁盘只有拖到池上并执行修改后才会
  进入对应层。

## V0.46 行高放宽下限 — 2026-09-07

### Changed
- 拓扑引擎的行高放宽上限改为 `max(5, H+1, ceil(H×1.3))`。兄弟行较矮时，池可以
  再收窄一些，再换到下一行。

### Verification
- 2026-09-07：`TopologyLayoutEngineTests` 通过 23，失败 0，跳过 0。

### Known Limitations
- 真实存储结构变更仍被拒绝。

## V0.46 编辑页下区结构工具与工作区恢复 — 2026-09-06

### Changed
- 产品版本为 **V0.46**。
- 管理页下次打开会恢复上次的存储系统和选中对象。Agent 已在运行时读取本地
  库存缓存，不再在界面启动时重新全量扫描。
- 编辑页下区卡片用右上角独立标记：有变更（强调色圆点）、被钉住（`E840`）、
  有数据（`ED43`）。有数据的池虚拟磁盘也会显示有数据标记。已去掉「未建立」
  虚拟磁盘占位卡。标记不占标题宽度。
- 下区右侧拆成存储结构（执行修改 / 创建新池、解散池、退出磁盘）和池属性
  （确认池属性，然后是参数表）。执行修改只提交成员进出。确认池属性只提交
  名称和参数。有数据时仍可改名，重建相关项会弹出提醒并拒绝。
- 退出有单独校验。系统盘 / 启动盘弹出提醒不能退出。页面文件盘 / 崩溃转储盘
  先确认去掉该角色，再进入未划层。退出后钉子去掉，可以再拖。仅模拟系统。

### Verification
- 2026-09-06：退出与状态标记的 EditWorkspace、模拟操作测试通过；新提交种类的
  架构检查通过；Release 重建后窗口已启动。

### Known Limitations
- 真实存储结构变更仍被拒绝。
- 退出对话框和成员提交目前只在模拟系统里生效。

## V0.45 设置恢复默认、缝隙阈值与数据容量 — 2026-09-05

### Changed
- 设置页新增"全部恢复默认"。必须经过确认对话框，绝不直接执行。它会把全部偏好
  （前台，及经 Agent 的后台项）恢复默认、把所有内置模拟系统恢复初始状态，用户
  自建的模拟系统不受影响。Agent 不可用时跳过后台部分并弹出警告通知。
- "分区忽略大小"更名为"分区缝隙隐藏阈值"，改为纯数字输入框，数值旁固定显示
  MiB 单位，去掉增减按钮；输入为空或非法时在失焦后回退到已存值。
- 新增后台偏好"数据容量限制"，默认 1024 MiB，范围 1 MiB–1 TiB，保存在
  agent-settings.json。它与数据压缩相关，当前没有任何消费行为；将在监控与测试
  功能中完整实现。

### Verification
- 2026-09-05：完整 Debug 自动门禁通过 397 个测试，0 失败，0 跳过。首轮验证期间
  Release 运行树因正在运行 WinPool 而无法重建。
- 2026-09-05（第二轮）：修复客户端响应解码缺失 `AgentPreferenceSavedResponse`
  的问题（此前 Agent 已持久化成功但 App 侧必然报错回滚）、恢复默认改为按递增
  Revision 重写内置模拟（持久层会拒绝全新目录文档直接覆盖）、数字框获得焦点
  全选并支持回车提交。实机 UI 验证通过：全部恢复默认完整执行无报错（前台文
  件、内置模拟、四项后台值含 1024 MiB 容量默认全部复位），容量输入改动完整
  落入 agent-settings.json。
...
### Changed
- 编辑页改为上下两半独立滚动，带行分割条，控件列固定宽度，去掉半区标题。
- 上半区显示磁盘/分区两级拓扑，未分配空隙按段拆开，并保留原有分区操作。
- 下半区显示内部池（含原始池）和新建池加号。物理磁盘在工作副本中于池间拖动，并自动进入 SSD、HDD 或 SCM 层。执行只提交模拟操作。
- 新增模拟操作：创建分层池、修改池参数、解散池并让磁盘回到原始池。

### Verification
- Application 测试覆盖分区/池投影、未分配空隙、自动分层、拒绝未知介质、分层创建、解散、以及多虚拟磁盘禁止修改。
- 架构测试要求编辑页使用这些投影和正式模拟协调器，并保持两个无标题半区。
- 2026-09-04：开发者确认原生编辑页点选 `passed`（本地、仅模拟）。验收时合并
  `main` 上完整 Release 自动门通过 369 个测试、0 失败、0 跳过，Release 构建
  0 警告、无已知漏洞依赖。阶段计划冻结于 `docs/Archive/V0.45`。

### Known Limitations
- 真实存储结构变更仍然禁止。
- 原生编辑页点选：实施时为 `unverified`；2026-09-04 经开发者确认 `passed`。
  继承的设备、UAC、DPI、操作系统矩阵和长时用例仍为 `unverified`。
...
- 启动时空的本机占位被缓存清单替换时，不再因旧选中身份不匹配而弹出“工作区初始化失败”。
