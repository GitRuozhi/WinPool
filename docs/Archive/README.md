...
归档记录历史，不定义当前要求。活动阶段见 [Plan](../Plan.md)（V0.52 实施中）。当前产品决定见 [Product](../Product.md)。旧设计的“已认可”是当时状态，不能覆盖后来的明确决定。

历史双语原件保持原样，不要求补译；本索引以中文单一版本维护。归档内的旧相对链接按对应归档说明或源提交追溯，不批量重写历史内容。

| 条目 | 状态 | 日期 | 版本/提交 | 内容 |
| --- | --- | --- | --- | --- |
| [V0.52 审查原稿](V0.52-review/README.md) | 原样保留；精简稿已获授权并转入唯一活动 Plan | 2026-09-14 | 基线 `85a3371` | 完整版和精简版，不作为并行执行规范 |
