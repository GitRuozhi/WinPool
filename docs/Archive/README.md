...
归档记录历史，不定义当前要求。当前无活动 Plan。当前产品决定见 [Product](../Product.md)。旧设计的“已认可”是当时状态，不能覆盖后来的明确决定。
...
| [磁盘分区编辑页-控件设计.md](磁盘分区编辑页-控件设计.md) | 未采纳的分区页草图；不是需求 | 2026-09-07 | 未跟踪原稿，V0.48 结束时归档 | 右下属性区草稿；与现行统一草稿/Volume/立即写入禁止项冲突，仅作对照 |
