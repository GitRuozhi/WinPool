...
| [`V0.45-layout-engine/`](V0.45-layout-engine/) | implemented per the developer's execution order; targeted tests and native verification recorded in the stage; CHANGELOG entry pending developer acceptance | 2026-09-06 | V0.45; implementation `51ed567`…`93e775e` on `main` | Unified topology layout engine: engine-owned slot allocation, per-surface viewport state, edit-upper capacity strips with three-stage allocation, edit-lower projection alignment, modifiability indicator with guardrails, width-adaptive disk headers |
