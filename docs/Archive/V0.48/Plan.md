...
- 状态：**已验收并归档**。2026-09-10 用户确认带成员的建池/改池可用，并要求结束本阶段。无后续活动 Plan。
- 执行起点：`e6b0a0ed8685fcd7328e41ee335837a62e6139ff`（文档重构 + 活动 Plan）。代码基线 `39f0d4eb3c141b169632976e598754b299a5a997` 为其父提交。
- 实现提交：`507fd42`。产品版本 **V0.48**。内部格式：schema 15 / IPC 5 / document 2 / snapshot 3。
- 自动门：Release 测试 460 通过；依赖审计无已知漏洞包。真实存储结构修改继续拒绝。
- 人工：用户确认带成员的建池与改池。WinUI 自动化覆盖两编辑页打开、草稿撤销/重做、扩缩容拒绝、双语、Manage 重载。完整设备、DPI、高对比度、托盘与长期运行仍为 **unverified**，不得改写成 passed。
- 已知未修交互（不阻塞关闭，不另起 Plan）：空池无成员直接应用会拒绝但无原因对话框，且草稿被丢；分区页撤销/应用按钮空闲时仍显示可用。
- 本阶段取代原 V0.47 控件 Plan。原件见 [文档重构归档](../20260910-documentation-reset/README.md)。
- `temp/磁盘分区编辑页-控件设计.md` 是用户未跟踪文件，保留，不纳入提交或默认作为需求。

本文件冻结于 `docs/Archive/V0.48/`。当前无活动 Plan；长期规则归 Product / Development / Quality。
...
| R6 | 完整回归与验证；受影响测试项目及整个 solution | §4 回归与 §5 自动门；报告实际覆盖和未验证项，禁止用静态字符串检查替代编辑行为 | 已验证：Release 自动测试 460 通过；用户确认建池/改池；完整设备 unverified |
...
用户确认结束本阶段。自动完成与人工验收分开记录。

| 项 | 结果 | 证据 |
| --- | --- | --- |
| 两编辑页打开 | passed | WinUI 自动化；用户手工 |
| 建池/改池完整应用（带成员） | passed | 用户手工确认 |
| 空池无成员应用 | 拒绝正确；反馈 failed | 预览可点确定；无原因对话框；草稿被丢。已知限制 |
| 分区页打开与扩缩容拒绝 | passed | WinUI：操作不可用 + supported-size 说明 |
| 撤销 / 重做 | passed | 结构页创建池草稿 |
| 软件双语 | passed | 设置切 English 后结构页为 Create pool / Undo；已改回中文 |
| Manage 重载 | passed | 仍为 3 存储池 / 14 物理盘；空池未持久化 |
| 估算显示 | passed（容量数字）；来源标签 unverified | Manage 有 TiB/GiB；1% 政策标签未在 UI 观察到 |
| Release 自动测试 | passed | 460 项，0 失败 |
| 完整设备 / DPI / 高对比度 / 托盘 / 长期 | unverified | 本阶段未启动 |

已知留下、不阻塞关闭的交互：

1. 结构页应用失败只处理 `OutcomeUnknown`；普通拒绝不弹原因并重置工作草稿。
2. 分区页撤销/重做/放弃/应用未按草稿启用，空闲时按钮仍亮。
