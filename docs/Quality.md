...
本文件规定验证选择与结果含义。有活动阶段时，具体用例和进度归 `docs/Plan.md`。当前 V0.51 磁盘编辑、入口一致性与布局修复代码已实施；2026-09-11 的后续缺陷、MAX 及五项补充修复已通过 Application 209 项、Architecture 43 项、全套 507 项测试及完整 Release 构建。含数据池的 MAX 按钮、脱机池属性只读、空池创建虚拟磁盘与分区的待办和应用状态已用原生 WinUI 验证；换行均分由实际拓扑布局引擎用例验证。其它人工视觉和真实设备场景仍未验证。V0.50 的历史验证和未验证状态见归档，不能替代本次验证。文档位置和语言规则归 [Development](Development.md)，真实操作边界归 [Product](Product.md)。
