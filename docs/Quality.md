...
本文件规定验证选择与结果含义。有活动阶段时，具体用例和进度归 `docs/Plan.md`。当前 V0.53 自动门为全套 550/550 测试、完整 Release 构建 0 警告/0 错误。隔离原生 WinUI 已验证开发者模式默认关闭、三页同时显示/隐藏、硬件位于管理之前及设置跨完全重启保留；硬件页还验证了固定项名、自动且有上限的设备列、分离的横纵滚动条、整页纵向滚动、设备列选择后自动居中与文本选择、无详情弹窗，以及标题栏在模拟系统和本机之间往返切换。网络来源整组刷新后由旧版 46 个对象收敛为 7 个，GPU 从混合来源的 7 列收敛为 4 个 DXGI Adapter，Monitor 从混合来源的 4 列收敛为 2 个 DXGI Output。硬件报告、统一存储摘要与本机只读补充采集的有限原生结果见[硬件报告实施核对](Reference/20260915_硬件报告实施核对.md)；当前机器只覆盖 96 DPI，192 DPI 为 `unverified`。更早 V0.52 统一事实与模拟编辑证据见 [V0.52 实施核对](Reference/V0.52-实施核对.md)。文档位置和语言规则归 [Development](Development.md)，真实操作边界归 [Product](Product.md)。
