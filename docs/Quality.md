...
本文件规定验证选择与结果含义。有活动阶段时，具体用例和进度归 `docs/Plan.md`。当前 V0.50 即时改名与结构草稿修复代码已实施；本轮禁止自行运行测试、构建或程序，因此所有新用例和组合场景仍为未运行。初次实施的自动门及原生界面检查结果见 [V0.50 归档](Archive/V0.50/README.md)，不能替代本次验证。文档位置和语言规则归 [Development](Development.md)，真实操作边界归 [Product](Product.md)。
...
V0.50 不用真实写操作验证模拟规则。未来允许真实写入的阶段，仍须按 Product/AGENTS 取得准确授权并记录目标与结果；自动测试和 CI 不触碰真实结构。缺少设备或目标平台时明确未验证，不制造通过记录。
