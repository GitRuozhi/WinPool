...
便携成品必须保持为一个完整目录。运行 `WinPool.App.exe` 时，`Agent`、`TestWorker`、
...
正常启动使用 Agent 独占的 SQLite 保存采集、工作区状态、模拟文档、监控、测试历史、证据和恢复数据。只创建或打开当前 schema；更旧 schema 会被拒绝且不迁移、不改写。当前 schema 修订号和 IPC 协议号记录在 [变更记录](CHANGELOG.zh-CN.md) 的 Compatibility 中，不构成额外项目版本。
...
产品代码不得静默擦除未知数据根。`storage-location.json` 是唯一耐久启动例外，因为 WinPool 必须先定位活动数据根，才能打开前述两个权威来源。IPC endpoint 是可重建运行状态；诊断和受管工具载荷是具有固定目录与保留规则的辅助文件，不是新的状态权威。
...
```text
artifacts\$(Configuration)\     四进程运行树
artifacts\obj\                  编译中间文件
artifacts\build\                类库和测试输出
```

`src`、`workers` 和 `tests` 只保留源码。App、Agent、Worker、Broker 构建后不再互相复制。
