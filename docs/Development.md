...
完整硬件刷新在既有 CIM/WMI 事实后追加 `WindowsGraphicsFactCollector` 和 `WindowsNetworkFactCollector`：前者以 DXGI LUID 保存适配器和输出，并用 D3D12 读取功能级别；后者以 `MSFT_NetAdapter` 的 `ConnectorPresent -or InterfaceType -ne 0` 作为网络对象集合边界，按接口索引关联全部 IP 地址与默认路由。统一层替换采集脚本产生的同类原始观察，并在刷新合并时把旧 `WinPool.NetworkAdapter` 来源视为同一已替代来源，避免当前采集和历史缓存形成两组对象。GPU 不按软件标志过滤，Monitor 不在报告投影中筛除，存储摘要不增加硬件页专用条件。
