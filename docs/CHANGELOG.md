...
内置模拟布局改为可展示、可测编辑器边界的一组：建池原料、标准两层池、空池、单盘池、热备与退役、双虚拟磁盘、分区边界、其它与网络；保留 DESKTOP-PL96UKD。分区偏移累加、盘符全局唯一、层介质与热备 Usage 对齐。去掉仅用于撑布局的 8 分区系统盘、16 盘无层、8 个小池、12 网络盘等样例。
