...
审查 `fde06b2` 后发现并修复：改名隐含模拟格式化、默认新建草稿被零容量拒绝、缩小层后分区越界、新 VD 容量未随层更新、切换选择丢创建意图、解散覆盖移动目标、Interleave 回填错误、动作对象/风险/父原因缺失，以及 MAX 撤销不同步。格式化现为独立动作；页面按池保存创建意图和 MAX 状态；规则、预检查和模拟保存共同保护最终结构。Debug/Release 各 489 项测试通过，原生空草稿与阻塞原因已检查；其余人工边界见活动 Plan。
