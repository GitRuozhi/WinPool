...
- 分区页改为格式化/卷属性工作区：动作条在拓扑左下横排，右侧属性跟随选中对象；去掉撤销/重做/放弃/应用全部。每次允许的动作单独经 Agent 提交，弹出成功或失败。未初始化磁盘不画分区；选中后点新建分区会提示先初始化。未分配上的格式化按钮改为创建新分区；本程序新建分区自动格式化，已有未格式化分区仍显示。
