...
本文件是当前产品行为、边界和路线的中文权威。2026-09-10 用户决定已覆盖相冲突的历史设计。当前实现为 V0.50；初次实施记录见 [V0.50 归档](Archive/V0.50/README.md)，九项缺陷修复见活动 [Plan](Plan.md)。已知限制见 [CHANGELOG](CHANGELOG.md)。

结构页的池/虚拟磁盘改名不会隐含格式化；格式化和卷重命名是独立动作。缩小层容量时，任何分区超过新的 OS 磁盘边界都会阻止整批应用。页面按池保存尚未应用的创建与 MAX 意图，切换对象及撤销重做不会改变其他池的计划语义。
...
| 提交方式 | 结构页：成员和参数属于同一次草稿；只读“待应用动作”显示合并后的净差异和规则原因，预览与应用使用同一计划。分区页：格式化与卷属性及时生效，每次动作单独提交并弹出成功或失败 |
