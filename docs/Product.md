...
本文件是当前产品行为、边界和路线的中文权威。2026-09-10 用户决定已覆盖相冲突的历史设计。当前实现为 V0.50；初次实施记录见 [V0.50 归档](Archive/V0.50/README.md)，最新即时改名决定与后续 BUG 修复见活动 [Plan](Plan.md)（代码未实施）。已知限制见 [CHANGELOG](CHANGELOG.md)。

2026-09-11 新决定（待实施）：已有对象在名称输入框按回车单独提交改名、即时生效（不增加保存名称按钮，普通失焦不提交），不参与结构动作或结构撤销/放弃；名称仅用于显示，身份和关联使用可靠 ID。尚未创建对象的名称属于创建参数。上轮已加入改名与格式化分离、空草稿占位和缩盘边界检查，但选择切换、意图撤销及解散原因仍有缺陷，不能视为全部收口。
...
当前实现版本为 V0.50；公开能力见根目录 [README](../README.zh-CN.md)。结构页提供净差异动作面板、MAX 容量意图和整批预检查；分区页仍及时提交。不包含任何真实存储写操作。
...
| 自动分区 | 建池流程默认可自动创建一个用户分区；分区页新建分区自动格式化。额外分区属于磁盘分区页，不扩大建池向导。已有未格式化分区仍显示 |
...
| 提交方式 | 结构页：成员和非名称参数属于同一次草稿；已有对象改名独立及时提交（新决定待实施），不受结构撤销/放弃影响；只读“待应用动作”显示合并后的净差异和规则原因，预览与应用使用同一计划。分区页：格式化与卷属性及时生效，每次动作单独提交并弹出成功或失败 |
