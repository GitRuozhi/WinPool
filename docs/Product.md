...
本文件是当前产品行为、边界和路线的中文权威。2026-09-10 用户决定已覆盖相冲突的历史设计。当前实现为 V0.49。活动阶段见 [Plan](Plan.md)（V0.50，待用户批准，尚未实施）；V0.49 计划与未验证项见 [归档](Archive/V0.49/README.md)。已知限制见 [CHANGELOG](CHANGELOG.md)。
...
当前实现版本为 V0.49；公开能力见根目录 [README](../README.zh-CN.md)。模拟语义、分区页及时生效与格式化属性已落实，不包含任何真实存储写操作。
...
| 提交方式 | 结构页：成员和参数属于同一次草稿，预览后统一应用。分区页：格式化与卷属性及时生效，每次动作单独提交并弹出成功或失败 |
