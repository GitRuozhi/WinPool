...
本文件是当前产品行为、边界和路线的中文权威。2026-09-10 用户决定已覆盖相冲突的历史设计。V0.48 已落地。活动阶段见 [Plan](Plan.md)（V0.49 分区页，已编制、待批准，不是开工指令）。已知限制见 [CHANGELOG](CHANGELOG.md)。
...
当前实现版本为 V0.48；公开能力见根目录 [README](../README.zh-CN.md)。V0.48 已落实模拟语义、相关引擎、持久化及缺陷修复，不包含任何真实存储写操作。
...
| V0.49 | 活动 Plan：磁盘分区页及时生效与格式化属性；待批准；无真实存储写操作 |
