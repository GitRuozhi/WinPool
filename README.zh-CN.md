...
- 在 1 MiB 网格上创建模拟 GPT 分区；创建容量使用整数 MiB，默认填入对齐后的最大容量，并提供 MAX 按钮。右侧分别提供“新建”和“格式化”。
...
“测试”页保留路线说明；开发页左上以单行列表显示最近 200 条本次运行消息，双击条目打开可复制的详情文本，点击浮层外关闭。右上和下方暂留空，下方提示人工智能入口正在开发。该页仅在开发者模式下可见。消息只保存在内存，退出 App 后清空；软件不查看日志文件内容，不记录完整操作历史。完整测试与开发工作区计划在 2.0 推出。
