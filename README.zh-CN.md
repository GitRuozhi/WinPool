...
- 监控受支持设备，持续显示会话运行时长，仅在有问题时逐项单行提示。
...
“测试”页保留路线说明；开发页提供可复制的诊断日志路径和最近 200 条“本次运行消息”，仅在开发者模式下可见。消息只保存在内存，退出 App 后清空；软件不查看日志文件内容，不记录完整操作历史。完整测试与开发工作区计划在 2.0 推出。

即时消息统一使用右下角简洁卡片，独立重复消息不合并。普通消息点击消失，错误消息点击打开消息对话框；两者都会自动消失，错误保留时间较长。详细记录在开发页查看。
