...
设置中的开发者模式默认关闭；启用后显示硬件、测试、开发三页，硬件排在管理之前。只读硬件页按十个大类呈现基本硬件报告，支持设备属性详情、本机完整刷新和完整系统 JSON 导出。当前格式为文档 3 / SQLite 17 / IPC 9，保存来源事实；旧格式明确拒绝，不迁移或删除旧数据。

管理页和硬件页启动先显示上次本机数据。Agent 启动后自动先采集存储系统、再采集完整硬件，分别回报更新；两页刷新按钮仍可手动触发对应采集。采集失败保留上次数据。
