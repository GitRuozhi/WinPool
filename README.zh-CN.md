...
当前实现版本为 **V0.53**。真实存储结构修改尚未开放。

设置中的开发者模式默认关闭；启用后显示硬件、测试、开发三页，硬件排在管理之前。只读硬件页按十个大类呈现基本硬件报告，支持设备属性详情、本机完整刷新和完整系统 JSON 导出。当前格式为文档 3 / SQLite 17 / IPC 10，保存来源事实；旧格式明确拒绝，不迁移或删除旧数据。
...
整个 1.x 的“测试”“开发”页面只显示路线说明，并仅在开发者模式下可见；完整工作区计划在 2.0 推出。
