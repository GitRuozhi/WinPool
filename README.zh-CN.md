...
管理页和硬件页启动先显示上次本机数据。Agent 启动后自动先采集存储系统、再采集完整硬件，分别回报更新；两页刷新按钮仍可手动触发对应采集。自动采集和手动刷新均在窗口内显示采集中、成功或失败提示；采集失败保留上次数据。读取历史不作为一次采集成功提示。
...
“测试”页保留路线说明；开发页左上“日志”列出最近 200 条本次运行消息，双击条目可查看并复制详情，仍提供可复制的诊断日志路径。右上和下方暂留空，下方提示人工智能入口正在开发。该页仅在开发者模式下可见。消息只保存在内存，退出 App 后清空；软件不查看日志文件内容，不记录完整操作历史。完整测试与开发工作区计划在 2.0 推出。

即时消息统一使用右下角固定大小的简洁卡片，退出时向右移动，独立重复消息不合并。普通消息点击消失，错误消息点击打开消息对话框；两者都会自动消失，错误保留时间较长。详细记录在开发页查看。
