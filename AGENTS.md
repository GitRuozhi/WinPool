...
- 开发阶段默认直接使用项目标准 `artifacts/Release` 运行树：按需关闭已核实的 WinPool App / Agent，修改或重建 WinPool 开发数据，并直接重建产物。无需为了保留正在运行的旧实例而创建隔离输出树，也不为这些已授权的开发动作反复请示。只有用户明确要求并行保留版本，或标准目录确实不能使用时，才另行处理输出位置。
...
- 主代理与每个子代理都应在实际运行时，根据当前 Agent 环境可用的模型、任务所需能力及性价比自主选择；不同 Agent 环境分别判断，不假设模型可互换或已配置切换。主代理通常选足以承担需求判断、任务拆分、协调和最终验收的强模型；子代理优先选择当前环境中能胜任工作且性价比较高的模型。可用模型或性价比变化时按实际情况调整，不要为了遵循示例而选择不可用、能力不足或已过时的模型。当前选择示例：Codex 主代理优先 6-Astra，其次 6-Sol；子代理优先 6-Luna，其次 6-Sol；不使用 5.6 系列模型。Z-Code 主代理优先 GLM-5.3，子代理优先 GLM-5.3-flash。Grok build 主代理优先 grok-4.7，子代理优先 grok-4.7-fast。示例只表达优先顺序，不代表这些模型在每次运行中都可用，也不代表已经完成运行时配置。
...
- 按模块和文件明确所有权，避免多个代理同时修改同一文件；共享构建输出串行使用，原生桌面操作同一时间只由一个代理负责。重建标准运行树前先关闭占用它的 WinPool 进程。
...
- 用户已长期授权：开发需要时可自主停止 WinPool App / Agent 进程，修改或重建 `%USERPROFILE%\AppData\Local\WinPool` 内的开发数据，并直接重建标准产物，无需每次请求批准。优先正常退出，确有需要可终止已核实路径的 WinPool 进程；整体清理数据前确认进程停止、解析并核对准确目录，按文件处置规则移入项目根 Rubbish 保留可恢复副本。此授权不涵盖其他数据目录、其他程序或真实存储结构修改，不要求每次开发都清理数据。
...
- 开发阶段可直接重建标准生成产物；普通清理不顺带运行会删除输出的重建脚本。使用完整重建脚本前核对其额外动作和目标范围。构建和运行目录替换按 [Development](docs/Development.md)处理。
