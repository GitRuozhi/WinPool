...
public sealed class LocalizationService : ObservableObject
{
    ...
    private static readonly IReadOnlyDictionary<string, (string Zh, string En)> Texts =
        new Dictionary<string, (string Zh, string En)>(StringComparer.Ordinal)
        {
            ...
            ["WelcomeMessage"] = (
                "WinPool 可浏览本机存储拓扑、编辑模拟系统，并在明确选择的测试目录中执行测试和持续监控。真实存储结构修改仍受严格的目标核验、风险分级和确认流程保护。",
                "WinPool can browse local storage topology, edit simulated systems, and run tests and continuous monitoring in explicitly selected test directories. Real storage-structure changes remain protected by strict target verification, risk classification, and confirmation flows."),
